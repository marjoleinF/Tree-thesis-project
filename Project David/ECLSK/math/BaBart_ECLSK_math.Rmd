...
data <- readdata[readdata$CHILDID %in% sample(x = unique(readdata$CHILDID), size = 240),]
...
for (k in 1:nreps){
  ...
  for (i in 1:nfolds){
    ...
    glmtree <- lmertree(score ~ 1 | (1|CHILDID) | GENDER + RACE + WKSESL + 
    ...
    for(ngen in ngensizes){
      ...
      if(use_weights){
        babart <- lmertree(y_gen ~ 1 | (1|CHILDID) | GENDER + RACE + WKSESL + 
                             C1GMOTOR + C1FMOTOR + T1INTERN + T1EXTERN + 
                             T1INTERP + T1CONTRO + P1FIRKDG + AGEBASELINE,
                         data = surr_mlbart, 
...
```{r}
# save MSE as CSV
write.csv(x = MSE, file = paste("MSE","Reading",if(baygen){paste0("baygen[",paste(ngensizes, collapse = ","),"]")}else{paste0("smearing[",paste0(palt*100,"%", collapse = ","),"]")}, if(use_weights){"weighted"}, sep = "_"))

# save tree_size as CSV
write.csv(x = tree_size, file = paste("TreeSize","Reading",if(baygen){paste0("baygen[",paste(ngensizes, collapse = ","),"]")}else{paste0("smearing[",paste0(palt*100,"%", collapse = ","),"]")}, if(use_weights){"weighted"}, sep = "_"))
```
