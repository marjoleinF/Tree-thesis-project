...
rm(list=setdiff(ls(), c("allMSE", "allTreeSize", "allMSElong", "allTreeSizelong")))
```

# Plot tree size for every dataset
...
# Convert MSE to Rsq, and add to allMSElong
...
## plot Rsq for ecery dataset, boxplots + violins
...
# find highest Rsq, Statistical analyses, anova + tukey
...
# statistical tests: repeated measures ANOVA
ANOVA_list_repeatedmeasures <- Rsquared_best_total %>% 
  dplyr::select(-Rsquared.Mean, -Rsquared.SD) %>% 
  split(f = .$Dataset) %>% 
  map(~ (.) %>% dplyr::select(-Dataset) %>%
        stats::reshape(varying = c(2:101), v.names = "Rsquared", idvar = "Model", timevar = "fold", direction = "long") %>% 
        rstatix::anova_test(., dv = Rsquared, wid = fold, within = Model) %>%
...
post_hoc_paired <- Rsquared_best_total %>% 
  dplyr::select(-Rsquared.Mean, -Rsquared.SD) %>% 
  split(f = .$Dataset) %>% 
  imap(~ (.x) %>% dplyr::select(-Dataset) %>%
         stats::reshape(varying = c(2:101), v.names = "Rsquared", idvar = "Model", timevar = "fold", direction = "long") %>%
         rstatix::pairwise_t_test(formula = Rsquared ~ Model,
                                            paired = T,
                                            p.adjust.method = "bonferroni",
                                            comparisons = list(c("GLMM tree",as.character(unique(.x$Model))[3]),
                                                               c("GLMM tree",as.character(unique(.x$Model))[4]),
                                                               c(as.character(unique(.x$Model))[3],as.character(unique(.x$Model))[4]))) %>% 
         dplyr::mutate(p.ADJ = p.adj*7) %>% # also adjust for multiple datasets
         rowwise() %>% 
         dplyr::mutate(p.ADJ.signif = stars(p.ADJ))
  )
...
# boxplots and GGbetweenstats, save to coolplots.Rdata
