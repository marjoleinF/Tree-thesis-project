...
##Bayesian data generation & smearing functions

```{r Bayesian Smearing Data generation}
...
if(baygen){
  tree_size <- MSE <- as.data.frame(matrix(data = NA, nrow = nfolds*nreps, ncol = 2 + length(ngensizes) + length(palts)))
  colnames(tree_size) <- colnames(MSE) <- c("GLMM_tree", "Bart", "BA",
                                            paste0("BaBart_N*",ngensizes), 
                                            paste0("Smearing_N*",expand.grid(ngensizes,palts)[,1], "_palt=",expand.grid(ngensizes,palts)[,2]),
                                            )
