...
#first run once to test the code:
  ...
  control_evtree <- evtree.control(maxdepth = maxd,minbucket = 10, minsplit = 20,  alpha=0.0001) #also see paper of evolutionary trees
  ...
  ##maybe we can increase the number of categories (also see paper by Os et al.)
...
#rpart
  ##Choose the right hyperparameters for rpart For example
  control2 <- rpart.control(maxdepth = maxd,minbucket = 10, minsplit = 20, xval=0,   cp=0.0001)
  model_rpart<-rpart(medv~., data=train, control=control2)
  result_mat_rpart[i,test_index] <- predict(model_rpart, newdata= test)
#BART
  ##set maximum size of the tree? or use the defaults? is there a minbucket??
...
for (i in 2:k) {
 ...
 #rpart
  ...
  model_rpart<-rpart(medv~., data=train)
  result_mat_rpart[i,test_index] <- predict(model_rpart, newdata= test)
  #bart
  #BART
  result_mat_bart[i,test_index]<-bart2(medv~.,data=train,test = test)$yhat.test.mean
